...
def check_size_t():
    ...
    for type, feature in [
        ('int', 'SIZE_T_IS_INT'),
        ('long', 'SIZE_T_IS_LONG'),
        ('unsigned int', 'SIZE_T_IS_UNSIGNED_INT'),
        ('unsigned long', 'SIZE_T_IS_UNSIGNED_LONG'),
        ]:
        file = open(name, "w")
        file.write(program % type)
        file.close()
        print "Check if 'size_t' and '%s' are the same type." % type
        os.system(build)
        try:
            os.remove(exe)
            print "YES"
...
def main(argv):
    ...
    # check if size_t is unsigned or unsigned long.
    check_size_t()
