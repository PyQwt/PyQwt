...
\newcommand{\OldSip}{\ulink{sip-3.11.1.tar.gz}
  {http://pyqwt.sourceforge.net/support/sip-3.11.1.tar.gz}}
...
\newcommand{\PyQwtTarGz}{\ulink{PyQwt-4.1.tar.gz}
  {http://prdownloads.sourceforge.net/pyqwt/PyQwt-4.1.tar.gz?download}}
...
\newcommand{\PrerequisitesEnd}{
  The source package \PyQwtTarGz{} contains the sources of the \Qwt-4.2.0
  ...
  However, the \file{configure.py} script lets you build PyQwt against a
...
\newcommand{\Future}{
  \begin{notice}[warning]
    The documentation is for the future PyQwt-4.2 which is only available from
...
\begin{itemize}
\item
  it really shines with either \module{Numeric}, \module{numarray} or both.
  Numeric and/or numarray extend the Python language with new data types which
...
\item
  ...
  Numeric, or numarray or both) and optionally SciPy in a GUI Python
  application or in an interactive Python session.
...
\begin{enumerate}
  ...
  \item
    ...
    \item
      However, \file{setup.py} offers the facilities to make an installer (.rpm
...
\subsection{Build prerequisites for PyQwt on \POSIX{} \label{posix-prereqs}}
...
\begin{enumerate}
  ...
  \item
    ...
    Supported versions: sip-4.1.1, -4.1, -4.0.1, -4.0,
...
\subsection{Build prerequisites for PyQwt on MacOS/X \label{macosx-prereqs}}
...
\subsection{Build prerequisites for PyQwt on Windows\label{win-prereqs}}
...
\begin{enumerate}
  ...
  \item
    \NewSip{} (requires at least Python-2.3.0) or \OldSip{}.\\
    Supported versions: sip-4.1.1, sip-4.1, -4.0.1, -4.0,
    -3.11, -3.10.2, -3.10.1, and -3.10,
    but the most recent version gets most testing.
    You may also try a \snapshot{} at your own risk.
  \item
    \PyQtCom{} or \PyQtEdu{} (or the academic or evaluation releases of
    PyQt). Install the SIP tools in case of a PyQt installer.\\
    Supported versions: PyQt-3.13, -3.12, -3.11, and -3.10
    but the most recent version gets most testing.
    You may also try a \snapshot{} at your own risk.
  \item
...
\item
  Configure PyQwt by running the following commands:
\begin{verbatim}
cd PyQwt-4.1
cd configure
python configure.py
\end{verbatim}
  ...
  options.
\item
  The next step is to build PyQwt using your platform's make command (nmake
  for MSVC): 
\begin{verbatim}
make
\end{verbatim}
\item
  The final step is to install PyQwt by running the following command (nmake
  for MSVC):
\begin{verbatim}
make install
\end{verbatim}
...
\begin{notice}[note]
  New: PyQwt-4.1 can be linked against an external \Qwt{}-4.2.0 library (does
  not work with PyQwt and \Qwt{}-4.2.0rcX release candidates).
  Run for instance:
\begin{verbatim}
...
\end{verbatim}
  and mail the file \file{LOG.force} or \file{log.txt} to the \mailinglist{}.
\end{notice}
...
\begin{verbatim}
python setup.py run_sip --sip-file-dirs=../PyQt-x11-gpl-3.13/sip build
\end{verbatim}
...
\item
  \class{QwtArrayLong} for \ctype{QwtArray<double>}
\item
  \class{QwtArrayQwtDiMap} for \ctype{QwtArray<QwtDiMap>}
\item
  \class{QwtArrayQwtDoublePoint} for \ctype{QwtArray<QwtDoublePoint>}
\item
  \class{QwtArrayQwtKeyPattern} for
  \ctype{QwtArray<QwtEventPattern::KeyPattern>}
\item
  \class{QwtArrayQwtMousePattern} for
  \ctype{QwtArray<QwtEventPattern::MousePattern>}
\end{enumerate}

Those classes have at least 3 constructors -- taking QwtArrayLong as an
example:
\begin{enumerate}
\item
  \code{array = QwtArrayLong()}
\item
  \code{array = QwtArrayLong(int)}
\item
  \code{array = QwtArrayLong(otherArray)}
\end{enumerate}

QwtArrayDouble and QwtArrayLong have also a constructor which takes a
sequence of items convertable to a C++ double and a C++ long:
\begin{itemize}
\item
  \code{array = QwtArrayDouble(Numeric.array([0.0, 1.0]))}
\item
  \code{array = QwtArrayLong(Numeric.array([0, 1]))}
\end{itemize}

All those classes have 16 member functions -- taking QwtArrayLong as example:
\begin{enumerate}
\item
...
\verbatiminput{QwtArrayLongExample.txt}
