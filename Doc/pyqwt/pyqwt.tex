...
\newcommand{\Snapshot}{\ulink{snapshot}{http://pyqwt.sourceforge.net/snapshot}}
\newcommand{\NewSip}{\ulink{sip-4.0rc3.tar.gz}{http://pyqwt.sourceforge.net/support/sip-4.0.rc3.tar.gz}}
\newcommand{\OldSip}{\ulink{sip-3.10.1.tar.gz}{http://pyqwt.sourceforge.net/support/sip-3.10.1.tar.gz}}
\newcommand{\SipNcZip}{\ulink{sip-win-nc-3.8.zip}{http://pyqwt.sourceforge.net/support/sip-win-nc-3.8.zip}}
\newcommand{\PyQtGpl}{\ulink{PyQt-x11-gpl-3.11.tar.gz}{http://pyqwt.sourceforge.net/support/PyQt-x11-gpl-3.11.tar.gz}}
...
\newcommand{\NumericTarGz}{\ulink{Numeric-23.1.tar.gz}{http://prdownloads.sourceforge.net/numpy/Numeric-23.1.tar.gz?download}}
\newcommand{\numarrayTarGz}{\ulink{numarray-0.9.tar.gz}{http://prdownloads.sourceforge.net/numpy/numarray-0.9.tar.gz?download}}
\newcommand{\PyQwtTarGz}{\ulink{PyQwt-4.0.tar.gz}{http://prdownloads.sourceforge.net/pyqwt/PyQwt-4.0.tar.gz?download}}
\newcommand{\ReadMePatches}{\ulink{README}{http://pyqwt.sourceforge.net/patches/README}}

% Installers for MS-Windows
\newcommand{\PythonExe}{\ulink{Python-2.3.3.exe}{http://www.python.org/ftp/python/2.3.3/Python-2.3.3.exe}}
\newcommand{\NumericExe}{\ulink{Numeric-23.1.win32-py2.3.exe}{http://prdownloads.sourceforge.net/numpy/Numeric-23.1.win32-py2.3.exe?download}}
\newcommand{\NumArrayExe}{\ulink{numarray-0.9.win32-py2.3.exe}{http://prdownloads.sourceforge.net/numpy/numarray-0.9.win32-py2.3.exe?download}}
\newcommand{\QtNcExe}{\ulink{QtWin230-NonCommercial.exe}{ftp://ftp.trolltech.com/qt/non-commercial/QtWin230-NonCommercial.exe}}
\newcommand{\PyQtExe}{\ulink{PyQt-win-nc-3.8.exe}{http://pyqwt.sourceforge.net/download/PyQt-win-nc-3.8.exe}}
\newcommand{\PyQwtExe}{\ulink{PyQwt\_Qt230-3.8.win32-py2.3.exe}{http://prdownloads.sourceforge.net/pyqwt/PyQwt\_Qt230-3.8.win32-py2.3.exe?download}}

\newcommand{\Future}{
\begin{notice}[warning]
This documentation is for the future PyQwt-4.0. Check the \Download{}
page for the latest releases or try a \Snapshot{}.
\end{notice}
}
\newcommand{\NonCommercial}{
\begin{notice}[note]
\PyQwtExe{} is the latest ``non-commercial'' version of PyQwt on Windows for
\QtNcExe{} and \PyQtExe{}, because Trolltech and Riverbank Computing have
discontinued their non-commercial software.  Hopefully, this situation will
change with the availability of a new book on Qt programming including a
non-commercial version of Qt-3.2.
\strong{I will try to keep \PyQwtTarGz{} working with the commercial releases
of Qt and PyQt on Windows}, but the installation instructions for Windows may
be inaccurate or obsolete.  
\end{notice}
}
%\renewcommand{\Future}{}
...
The \ulink{Numeric}{http://www.numpy.org/index.html}
and \ulink{numarray}{http://stsdas.stsci.edu/numarray/index.html}
...
You can install PyQwt on \POSIX{} (Unix, Linux, Mac OS X) and Microsoft Windows
operating systems:
\begin{enumerate}
  \item
  ...
  \item
    ...
    from source code on \POSIX{} operating systems (Unix, Linux and Mac OS X).
  \item
    see \ref{win-source-install} for installation instructions starting from
    source code on Windows with MSVC and Qt-Non-Commercial.
    Thanks to feedback from developers with a commercial Qt license the
    installation scripts have been adapted to Qt-3.0.x, Qt-3.1.x,  Qt-3.2.x
    and Qt-3.3.x.
    Contact the \mailinglist{}, if you run into problems.
\end{enumerate}

\begin{notice}[note]
The \mailinglist{} is a subscribers-only list to reduce spam.  I appreciate
if you subscribe before posting to the list, since it saves me the work of
accepting or rejecting mail from non-subscribers.  You can always unsubscribe.
\end{notice}
...
The installation instructions in this section are for the Unix, Linux and
MacOSX operating systems.
The \file{configure.py} script works on more platforms than the \file{setup.py}
script and is recommended for normal installations.
The \file{setup.py} script has advantages for development and packaging.
 
\subsection{Prerequisites for building PyQwt on \POSIX{} \label{posix-prereq}}
...
\begin{enumerate}
  \item
    ...
    \PyQwtTarGz{} is compatible with Python-2.3.x, Python-2.2.x, and probably
    with Python-2.1.x.
    Earlier versions of Python are not supported.
  \item
    \ulink{Qt}{http://www.trolltech.com}.\\
    \PyQwtTarGz{} is compatible with Qt-3.3.x, Qt-3.2.x, Qt-3.1.x, Qt-3.0.x,
    Qt-2.3.1 and Qt-2.3.0.
    Earlier versions of Qt are not supported.
  \item
    \NewSip{} (requires at least Python-2.3.0) or \OldSip{}.\\
    \PyQwtTarGz{} is compatible with sip-4.0, -3.10.1, -3.10, and -3.9, but the
    most recent version gets most testing.
    You may also try a \snapshot{} at your own risk.
  \item
    \PyQtGpl{}.\\
    \PyQwtTarGz{} is compatible with PyQt-3.11, -3.10 and -3.9, but the most
...
The source package \PyQwtTarGz{} contains a version the \Qwt{} library based
on version 4.2.0rc2.


\subsection{Build on \POSIX{} using \file{configure.py}\label{posix-configure}}

\Future{}

\begin{notice}[warning]
The configuration module of PyQt-3.9 is broken, making it impossible to use 
the \file{configure.py} script of PyQwt.
Solutions are to upgrade to more recent versions of sip and PyQt or to 
to use the \file{setup.py} script as explained in \ref{posix-setup}.
\end{notice}
...
\subsection{Build on \POSIX{} using \file{setup.py}\label{posix-setup}}
...
The installation instructions in this section are for Windows.

The \file{configure.py} script allows you to compile PyQwt with a different
Visual C++ compiler than the compiler used for building Python.

Building PyQwt from source on Windows for Microsoft Visual C++ with the
\file{setup.py} script  produces a PyQwt*.exe installer at no cost!
...
\PyQwtTarGz{} should work with sip-3.9, sip-3.10, sip-3.10.1, sip-4.0,
PyQt-3.9-Commercial, PyQt-3.10-Commercial and PyQt-3.11-Commercial.
Install sip and PyQt with the new 'configure.py'
script instead of the deprecated 'build.py' script and make sure that the
*/*.sip files are installed in the location indicated by
\code{pyqtconfig._pkg_config['pyqt_sip_dir']}.
...
\subsection{Build on Windows using \file{configure.py}\label{win-configure}}
...
the \file{configure.py} script of PyQwt.
Solutions are to upgrade to more recent versions of sip and PyQt or to 
to use the \file{setup.py} script as explained in \ref{win-setup}.
\end{notice}
...
\subsection{Build on Windows using \file{setup.py} \label{win-setup}}
