...
\newcommand{\numarrayTarGz}{\ulink{numarray-1.1.tar.gz}
  {http://prdownloads.sourceforge.net/numpy/numarray-1.1.tar.gz?download}}
...
\begin{enumerate}
  ...
  \item
    ...
    It also allows you to make an installer (.rpm on some Linux systems and
    .exe on Microsoft Windows).
  \item
...
\end{verbatim}
  ...
  options.
\item
  The next step is to build PyQwt using your platform's make command.
\begin{verbatim}
make
\end{verbatim}
\item
  The final step is to install PyQwt by running the following command.
\begin{verbatim}
make install
\end{verbatim}
\end{enumerate}
...
\begin{notice}[note]
  The official PyQwt-4.1 release will allow you to build and link
